...
async def post_create(
    ...
    for friend_id, in rows:
        ...
        await request.state.hub.publish_to_user(friend_id, message)
    key = f'users:{user_id}'
    r.zadd(key, {post_id: int(datetime.now().replace(microsecond=0).timestamp())})
    r.zremrangebyrank(key, 0, -1001)
...
def post_feed(offset: int = 0, limit: int = 10, user_id: str = Depends(check_token)):
    r = get_reddis_conn()
...
async def post_feed_posted(ws: WebSocket):
    ...
    try:
        ...
        await ws.close()
        raise HTTPException(status_code=401, detail='Неавторизованный доступ')
    conn = Connection(ws, payload['user_id'])
    await ws.app.state.hub.connect(conn)
    try:
        await asyncio.Future()
    finally:
        await ws.app.state.hub.disconnect(conn)
