...
def get_id(id: str):
    ...
    select_user = '''
        SELECT id, first_name, second_name, birthdate, city, biography 
          FROM prod.users 
         WHERE id = %s
    '''
    with conn, conn.cursor() as cur:
    ...
    if not row:
        ...
        id, first_name, second_name, birthdate, city, biography = row
...
def search(first_name: str, second_name: str):
    conn = psycopg.connect(
        host=os.getenv('DB_HOST'),
        port=os.getenv('DB_PORT'),
        dbname=os.getenv('DB_NAME'),
    ...
    return [SearchResponse(
        ...
        city=city,
        biography=biography
    ) for id, first_name, second_name, birthdate, city, biography in rows]
